...
- 전제 조건·동작 방식 상세: `AIFAB_웹IDE_개발환경_구축방향_최종_v1-0.md` 3.4
...
> 도구별 상세 프로파일(할 수 있는 것/없는 것·적합 신호), 전체 결정 트리, 판별 질문 세트, 경계 사례 판정은 별도 기준 문서를 따른다: `AIFAB_가이드챗봇_트리아지_기준_v1-0.md` — 챗봇 Knowledge Base의 원천 문서
...
- 개발환경(웹 IDE)·Claude Code 비용은 별도: `AIFAB_웹IDE_개발환경_구축방향_최종_v1-0.md` 5장
...
- 운영 KPI: 신청(상담) 건수, 판정 유형별 비율(승인/미승인/보류·상용 툴 안내), 활성 과제 수, 격상 건수, 절감 공수, 평균 심사 소요, 선셋 회수 비용
...
- 가이드 챗봇으로 불필요한 환경 배정 원천 차단 — 자원을 신규 개발이 필요한 과제에 집중
- 계정 격리·가드레일로 실험의 리스크가 공식 환경에 전파되지 않는 구조
...
| `AIFAB_웹IDE_개발환경_구축방향_최종_v1-0.md` | 개발환경 확정안 (EC2+Coder) · LLM 백엔드 정책 |
| `AIFAB_가이드챗봇_트리아지_기준_v1-0.md` | 가이드 챗봇 도구별 프로파일·결정 트리·판정 규칙 (8장 상세) |
