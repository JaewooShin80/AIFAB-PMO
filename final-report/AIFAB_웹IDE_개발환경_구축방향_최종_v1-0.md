...
### 3.2 워크스페이스 (과제 승인 시 자동 생성 — **인당 1개**, 팀플랜 시트·자격증명이 개인 단위이므로)
...
| LLM 연동 | **코딩 작업: Claude 팀플랜 로그인**(개인 OAuth, 3.4 전제 조건 충족 시) / **에이전트·대체 경로: Bedrock 키리스**(IAM 롤, 승인 모델 ARN 한정) — 두 설정 모두 골든 AMI에 포함 |
...
### 3.4 LLM 백엔드 정책 — 코딩 작업은 팀플랜, 에이전트 호출은 Bedrock

Claude Code는 동일한 도구이고 백엔드만 다르다(`CLAUDE_CODE_USE_BEDROCK=1` 여부). **용도(사람의 코딩 작업 vs 시스템의 에이전트 호출)로 백엔드를 이원화**한다.

| 구분 | 코딩 작업 — **Claude 팀플랜** | 에이전트 호출 — **Bedrock** |
|:---|:---|:---|
| 대상 | 워크스페이스 내 Claude Code 사용 (시티즌 개발자·파워유저) | 심사 에이전트·리포트 에이전트 등 시스템 자동 호출 |
| 자격증명 | 개인 OAuth 로그인 — **프리미엄 시트 개인 단위, 공유 금지** | 키리스 (IAM 롤, 승인 모델 ARN 화이트리스트) |
| 데이터 경로 | Anthropic SaaS(사외) — 상용 플랜 기본 학습 미사용 | 자사 AWS 리전 내 유지 (VPC Endpoint) |
| 거버넌스 | Anthropic 관리 콘솔의 사용량 관리 (Bedrock 로깅·Guardrails 미적용 — 수용 필요) | 호출 로깅·Guardrails 전면 적용 (백서 7장) |
| 비용 | 인당 월정액 (사용 한도 있음) | 토큰 종량제 |

**워크스페이스 내 팀플랜 사용의 동작 방식** — 기술적으로 검증된 경로다:
- 로그인은 헤드리스 지원: 터미널 `claude` 실행 → 인증 URL 출력 → 사용자 PC 브라우저에서 팀플랜 로그인 → 코드 붙여넣기. 자격증명은 본인 워크스페이스 `~/.claude`에 저장
- 아키텍처 변경 불요 — IGW를 만드는 게 아니라 기존 통제된 egress 경로(화이트리스트)에 도메인을 추가하는 것이므로 SCP 가드레일과 충돌 없음

**채택 전제 조건 4가지:**

| # | 조건 | 내용 |
|:--|:---|:---|
| 1 | 아웃바운드 도메인 허용 | `claude.ai`·`api.anthropic.com` 화이트리스트 추가. NAT만으로는 도메인 필터 불가 — **AWS Network Firewall(SNI) 또는 명시적 프록시**로 FQDN 단위 허용 (E-1 구현 방식과 함께 확정) |
| 2 | 정보보호팀 승인 (E-2) | 프롬프트·코드의 사외 전송 승인 — 실데이터(마스킹) 취급 워크스페이스 포함 여부가 핵심 쟁점 |
| 3 | 시트·워크스페이스 인당 분리 | 시트는 개인 단위이고 OAuth 자격증명이 워크스페이스에 저장되므로 **워크스페이스는 인당 1개**로 생성 (Coder 자연 지원) — 팀 공유 시 계정 공유 발생 |
| 4 | 텔레메트리 최소화 | 비필수 외부 트래픽(통계·오류 보고) 차단 설정으로 화이트리스트를 최소 유지 |

- Bedrock 경로는 에이전트 외에도 **대체 경로로 유지**한다(골든 AMI에 양쪽 설정 포함) — 시트 미보유자·승인 지연 시 즉시 전환 가능
- 두 경로 모두 하네스·스킬 동작은 동일 — 백엔드 차이로 작업 방식이 갈라지지 않는다
...
- **팀플랜 채택 시(3.4)**: 코딩 작업 비용은 토큰 종량제 대신 **프리미엄 시트 인당 월정액**으로 대체되고, Bedrock 토큰 비용은 에이전트(심사·리포트) 호출분만 남음
