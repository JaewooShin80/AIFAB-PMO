...
| M4 | 골든 패스 완비 | 08-28(금) | 6대 템플릿(ECS·Lambda·RDS·비용태그·관측성·Secrets) 드라이런 통과 — 폼 입력→스테이징 배포 1시간 이내 | AI 인프라팀 |
...
| M7 | Gate 1 중간 판정 | 09-11(금) | 팀별 Go/Pivot/Stop 판정 (SLA 24~48h 내) | AI Board·멘토 |
...
| M11 | 이관/회수 완료 | 10월 중 | 이관: 7단계 격상 체크리스트 완료 / 폐기: Frozen→접근 회수(14일)→리소스 삭제(21일) | AI 인프라팀 |
...
| 항목 | 내용 |
|---|---|
| 활동 | ① 골든 패스 6대 템플릿 구축: ECS on Fargate·Lambda·RDS·비용태그·관측성(로그·메트릭 기본값)·Secrets Manager — 폼 입력만으로 CI/CD·IAM·모니터링 포함 레포 자동 생성 ② SAST(Semgrep 등)·시크릿 검출을 파이프라인에 내장(보안 내재화 — AI Board 전수 리뷰 부담 제거) ③ 3-lane 게이트 분류 기준·리뷰 절차 확정 ④ 서비스 카탈로그 위생 점검(카탈로그 부정확 상태에서 AI 도입 시 변경 실패율 +30%) ⑤ 드라이런: 비개발자 1명이 폼 입력→스테이징 배포 1시간 이내 검증 |
| 담당 | AI 인프라팀 (4~6주 전담 투입) |
| 산출물 | 골든 패스 템플릿 6종, 3-lane 분류 기준서, 드라이런 결과서 |
...
- **W2 (09-07~09-11)**: 개발 집중(회의 최소화) → Gate 1 중간 심사(09-11) → 24~48h 내 Go/Pivot/Stop 판정
...
| 출구 3방향 | **① 이관(Scale)**: 7단계 격상 체크리스트(코드 리뷰→문서화→실데이터 재설계→운영 조직 지정→재프로비저닝→이관·검증→원 환경 정리). 이관팀과 기술 부채 수용 기준(중복률·테스트 커버리지·취약점 0건) 사전 합의 필수 — 미합의 시 이관 보류 ② **폐기(Stop)**: 계정 Frozen → 14일 접근 회수 → 21일 리소스 삭제(비용 자동 통제). 최종 계정 폐쇄는 보드 심의로 결정 ③ **현업복귀(Redirect)**: 과제는 종료하되 참여자는 챔피언으로 전환, 백로그는 차기 코호트 후보로 등록 |
