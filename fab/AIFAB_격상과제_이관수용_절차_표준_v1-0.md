...
| 5 | **데이터 등급 전환(마스킹 해제)은 보안 절차 필수 경유** | 마스킹 데이터 → 민감정보 원본 전환은 설계 재검토와 정보보호팀 재심사(탑다운 기획안 4.1)를 반드시 거친다. 어떤 경우에도 샌드박스 설계 그대로 민감정보 원본을 연결하지 않는다 |
...
| 사전 스크리닝 | 재작성 필요 여부(컨테이너·파이프라인 이탈 구조), 마스킹 해제(민감정보 원본 전환) 범위를 심의 자료에 포함 |
...
| 3 | 데이터 연동 재설계(마스킹 해제) | 과제 오너 + 정보보호팀 | 데이터 흐름 재설계서 + 정보보호팀 재심사 승인 (탑다운 기획안 4.1) | 1~2주 |
...
| 데이터 명세 | 취급 데이터 등급·보존 기간·파기 절차 (마스킹 해제 재심사 결과 반영) |
