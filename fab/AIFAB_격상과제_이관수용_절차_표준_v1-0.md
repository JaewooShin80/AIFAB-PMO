...
| 6 | 이관·검증 | 과제 오너 + 보드 | Git 이관 → 스테이징 검증 통과 → 승인 게이트(보드·정보보호팀) 통과 | 1주 |
| 7 | 원 환경 정리 | AI 인프라팀(자동화) | 샌드박스 자원 회수·데이터 파기 확인 (선셋 절차 준용) — **운영 오픈 + 하이퍼케어 종료 후 실행** | 오픈 후 2주 시점 |

### 4.3 Stage C — 운영 전환

- 운영 오픈 시 SLO(가용성·오류율·응답시간)를 보드가 확정하고 모니터링·알람 자동 구성을 확인한다
...
| 항목 | 기준 |
|:---|:---|
| 정적 분석(SAST) | Critical/High 취약점 0건 (기준 상세는 정보보호팀 협의 확정) |
| 시크릿 | 하드코딩 시크릿 0건 — 전량 Secrets Manager 이관 |
| 테스트 | 핵심 비즈니스 로직 단위 테스트 존재, 커버리지 임계값 (협의 확정, 예: 핵심 모듈 60%+) |
