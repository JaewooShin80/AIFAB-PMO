# AIFAB 탑다운 파일럿 — 3주 스프린트 운영 시간표 (실무용)

> v1.0 | 2026-07-12 | 대상: AI Board·멘토·챔피언·시티즌 개발자 팀
...
flowchart LR
    ...
    W2 --> G1{{"Gate 1<br/>중간 판정<br/>9/11 심사<br/>SLA 24~48h"}}
    G1 -->|Go| W3["W3 완성·배포<br/>9/14~9/18"]
    G1 -->|Pivot| W3
...
| 9/2(수) D2 | **골든 패스 스켈레톤 생성**: 폼 입력 → CI/CD·IAM·모니터링 포함 레포 자동 생성 → 스테이징 배포 | 개발 착수 + 멘토 방향성 점검 #1 (1h) | **전 팀 스테이징 배포 성공** (당일 목표) |
...
> **Gate 1 판정 (AI Board·멘토 합의, SLA 24~48h — 9/14 월 오전까지 통보)**
> - **Go**: 계획대로 W3 진행
...
| 9/16(수) D12 | **Gate 2 심사 1부 — 자동 검사**: 단위 테스트, 시크릿 미검출, SAST Critical/High 0건 | **Gate 2 심사 2부 — 3-lane 코드 리뷰** (아래 §3) | 심사 결과서 |
| 9/17(목) D13 | **AI Board·정보보호팀 승인 게이트** (수동) | 스테이징 최종 검증 → **Blue/Green 프로덕션 배포** (실패 시 자동 롤백) | **운영 배포 완료** |
...
| 9/25(금) | 결과 공유회, 이관 팀 → 7단계 격상 절차 착수 / 폐기 팀 → 계정 Frozen 개시 | 파일럿 최종 보고 초안 |

## 6. 측정 KPI (킥오프 전 동결 — 8/28 확정)
...
- 전체 타임라인·시나리오: `AIFAB_탑다운_파일럿_구축_타임라인_운영시나리오_실무_v1-0.md`
