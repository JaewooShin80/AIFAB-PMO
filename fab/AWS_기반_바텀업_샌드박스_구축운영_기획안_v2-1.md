...
AWS Organization으로 Shared Services(공통), Top-down(공식 과제), Bottom-up Sandbox(자율 과제) 계정을 분리한다. Shared Services 계정에는 신청 포털과 승인 자동화(API Gateway, Lambda, Step Functions), Bedrock AI Agent, Terraform을 배치한다. 신청 포털에서 과제 구분(Top-Down/Bottom-up)을 선택하여 과제 내용을 기술하면 접수 처리 후 구분에 따라 흐름이 분기한다.

- **Top-Down(공식 과제)**: AI 사전심사를 바이패스하여 운영 보드(AI Board) 승인으로 직행한다. 승인 시 Terraform이 Top-down 계정에 개발 서버를 자동 세팅하며, 개발 완료 후 GitLab 기반 CI/CD로 배포한다. 배포 전 사내 정보보안 프로세스에 따른 확인을 거쳐 별도의 운영 서버에서 구동한다.
- **Bottom-up(자율 과제)**: Bedrock AI Agent의 적합성 사전심사를 거쳐 운영 보드가 승인하면, Terraform이 Sandbox 계정에 표준 스택(ECS/Fargate, EC2, S3, RDS)을 자동 생성한다. 과제 수행자는 세팅된 환경에 SSO로 접속하여 개발 서버에서 필요시 S3·RDS를 활용해 개발하고, 완성된 앱을 CI/CD를 거쳐 과제 앱(ECS/Fargate)으로 배포한다.

CloudTrail, GuardDuty, Security Hub, CloudWatch는 전 계정 공통의 보안·관측 계층으로 운영하며, 두 계정의 로그·지표를 모두 수집한다.

![그림 1. AWS 전체 아키텍처 (Top-Down / Bottom-up 신청·프로비저닝·개발 흐름)](media/fig1-aws-architecture.png)

*그림 1. AWS 전체 아키텍처 (Top-Down / Bottom-up 신청·프로비저닝·개발 흐름)*
...
생애주기 흐름: Terraform이 세팅한 환경에 SSO로 접속 → 개발 서버에서 필요시 S3·RDS를 활용해 개발 → CI/CD 파이프라인(7장) 통과 → 과제 앱(ECS/Fargate)에 배포 → 사내 사용자가 내부 ALB를 통해 접속.
